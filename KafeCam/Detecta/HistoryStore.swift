...
class HistoryStore: ObservableObject {
    ...
    func syncLocal() {
        ...
        debugLog("Historial cargado localmente (\(entries.count) entradas)")
    }
    ...
    func syncFromSupabase() async {
        ...
        do {
            ...
            debugLog("[HistoryStore] Syncing captures for user: \(userId)")
            ...
            debugLog("[HistoryStore] Found \(items.count) captures in database")
                ...
                for c in items {
                    group.addTask {
                        do {
                            ...
                            debugLog("[HistoryStore] Failed to fetch image for key \(c.photoKey): \(error)")
                        }
    ...
    private static func fetchImage(for capture: CaptureDTO, using storage: StorageRepository) async throws -> UIImage? {
        ...
        do {
            ...
            debugLog("[HistoryStore] Fetching image from: \(capture.photoKey)")
            let (data, resp) = try await URLSession.shared.data(from: url)
            guard let http = resp as? HTTPURLResponse, 200..<300 ~= http.statusCode else { 
                debugLog("[HistoryStore] Failed to fetch image, status: \((resp as? HTTPURLResponse)?.statusCode ?? -1)")
                return nil 
            }
            return UIImage(data: data)
        } catch {
            debugLog("[HistoryStore] Error fetching image for \(capture.photoKey): \(error)")
            return nil
        }
        #else
        return nil
