...
struct HistoryDetailView: View {
    ...
    private func saveNotes() {
        ...
        guard let captureData = entry.captureData else { 
            debugLog("[HistoryDetail] No captureData available - cannot save notes to database")
        ...
        Task {
            ...
            do {
                debugLog("[HistoryDetail] Saving notes for capture ID: \(captureData.id)")
                let repo = CapturesRepository()
                let updated = try await repo.updateNotes(captureId: captureData.id, notes: notes.isEmpty ? nil : notes)
                debugLog("[HistoryDetail] Notes saved successfully. Updated capture has notes: '\(updated.notes ?? "nil")'")
                ...
                debugLog("[HistoryDetail] Error saving notes: \(error)")
                await MainActor.run {
                    isSavingNotes = false
