...
final class ProfileTabViewModel: ObservableObject {
	...
	func load() async {
		do {
			...
			debugLog("[ProfileTabVM] Loading profile for User ID: \(userId)")
			
			let repo = ProfilesRepository()
			let p = try await repo.getOrCreateCurrent()
			
			debugLog("[ProfileTabVM] Profile loaded - Name: \(p.name ?? "nil"), Phone: \(p.phone ?? "nil"), Email: \(p.email ?? "nil"), Org: \(p.organization ?? "nil"), Role: \(p.role ?? "nil")")
			...
			debugLog("[ProfileTabVM] Error loading profile: \(error)")
		}
	}
	...
	func saveProfile(name: String?, email: String?, phone: String?, organization: String?,
		...
		do {
			...
			debugLog("[ProfileTabVM] saveProfile error: \(error)")
		}
		#endif
	}
	...
	func uploadAvatar(_ image: UIImage) async {
		...
		do {
			...
			debugLog("[ProfileTabVM] Avatar upload complete and metadata updated")
		} catch {
			debugLog("[ProfileTabVM] Avatar upload error: \(error)")
		}
		#endif
	}
